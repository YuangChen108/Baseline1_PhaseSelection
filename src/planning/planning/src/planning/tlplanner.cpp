...
// ✅ 将 wave_omega 作为一个传参放进来
ZPredictionRes get_external_z_prediction(double current_z, double current_vz, double estimated_T, double total_dur, double dt, double wave_omega) {
    ZPredictionRes res;
    double mock_z_mean = 0.0; 

    double dz0 = current_z - mock_z_mean;
    double vz0 = current_vz;

    // 直接使用传进来的 wave_omega 计算
...
TLPlanner::TLPlanner(std::shared_ptr<parameter_server::ParaeterSerer>& para_ptr):paraPtr_(para_ptr){
    ...
    paraPtr_->get_para("wave_omega", wave_omega_);
...
TLPlanner::PlanResState TLPlanner::plan_land(const Odom& init_state_in, const Odom& target_data, TrajData& traj_data, const double& t_replan) {
    ...
    double intercept_vz = z_pred_result.intercept_vz;
    // =======================================================================
    ...
    if (use_prediction_logic) {
    // 使用局部变量 intercept_vz
    final_state.col(1) = Eigen::Vector3d(tar.v_*cos(tar.theta_), tar.v_*sin(tar.theta_), intercept_vz);
    } else {
